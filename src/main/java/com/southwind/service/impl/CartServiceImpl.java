...
public class CartServiceImpl extends ServiceImpl<CartMapper, Cart> implements CartService {
    ...
    @Override
    public Integer getResult(User user, Integer productId) {
        Cart seckillCart = cartMapper.selectOne(new QueryWrapper<Cart>().eq("user_id",
                user.getId()).eq("product_id",productId));
        if (null!=seckillCart){
            return seckillCart.getId();
        }else if (redisTemplate.hasKey("isStockEmpty:"+productId)){
            return -1;
        }else {
            return 0;
        }
...
//    @Override
//    @Transactional
//    public Cart secKill(User user,Product product){
//        ValueOperations valueOperations = redisTemplate.opsForValue();
//        Product seckillproduct = productService.findByGoodsId(product.getId());
